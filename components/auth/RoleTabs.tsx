...
const RoleTabs: React.FC<RoleTabsProps> = ({
  ...
  activeRole,
  onRoleChange,
  primaryColor = 'aksess-purple', 
  accentColor = 'aksess-skyblue',     
  textColor = 'aksess-navy',         
  inactiveTextColor = 'gray-500', 
  borderColor = 'gray-200'        
}) => {
  // Construct dynamic Tailwind classes to ensure they are processed
  const activeBorderClass = `border-${accentColor.replace('#', '')}`; // e.g., border-aksess-auth-accent or border-F5AF47
  const activeTextClass = `text-${textColor.replace('#', '')}`;     // e.g., text-aksess-auth-primary or text-339999
