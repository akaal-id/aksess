...
export const UserMenuButton: React.FC<UserMenuButtonProps> = ({ 
  ...
  const userMenuButtonActiveStyles = `bg-aksess-purple/10 text-aksess-purple border border-aksess-purple hover:bg-aksess-purple/20`;
  const userMenuButtonInactiveStyles = `text-aksess-deep-navy hover:bg-aksess-purple/5`;
    ...
    <div ref={userMenuRef} className="relative w-full sm:w-auto">
      ...
      >
        <div className="flex items-center gap-2">
          <UserCircle size={20} className={isUserMenuOpen ? "text-aksess-purple" : "text-aksess-deep-navy group-hover:text-aksess-purple"} />
          <span className={`truncate max-w-[100px] sm:max-w-[120px] ${isUserMenuOpen ? "text-aksess-purple" : "text-aksess-deep-navy group-hover:text-aksess-purple"}`}>Hi, {userName}!</span>
        </div>
        <ChevronDown size={16} className={`transition-transform duration-200 ${isUserMenuOpen ? 'rotate-180' : ''} ${isUserMenuOpen ? "text-aksess-purple" : "text-aksess-deep-navy/70 group-hover:text-aksess-purple"}`} />
      </button>
        ...
        >
          <div className="py-1" role="none">
            <button
              ...
              className="w-full text-left flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-aksess-purple/10 hover:text-aksess-purple transition-colors"
              role="menuitem"
            >
...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const navbarBgColor = 'bg-aksess-white'; 
  const navbarBorderColor = 'border-b border-aksess-purple/10';
  const logoTextColor = 'text-aksess-purple'; 
  
  const mobileIconColor = `text-aksess-deep-navy hover:bg-gray-100`; 
  const mobileMenuBorderColor = 'border-aksess-chino';

  const authButtonStyles = `bg-aksess-purple/10 text-aksess-purple border border-aksess-purple px-5 py-2.5 rounded-full text-sm font-semibold 
                            hover:bg-aksess-purple/20 transition-colors flex items-center justify-center gap-2`;
  
  // Styles for the "Buat Event" button when user is NOT logged in
  const createEventNotLoggedInStyles = `bg-aksess-purple text-aksess-white border border-aksess-purple px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2 transform hover:scale-105`;
  const createEventNotLoggedInMobileStyles = `w-full text-center bg-aksess-purple text-aksess-white border border-aksess-purple px-5 py-3 rounded-full text-sm font-semibold hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2`;
  ...
  if (isLoggedIn) {
    ...
    switch (userRole) {
      case 'visitor':
        ...
        dynamicButtonStyles = `bg-aksess-purple/10 text-aksess-purple border border-aksess-purple hover:bg-aksess-purple/20 ${baseModeButtonStyles}`;
        break;
      case 'creator':
        dynamicButtonText = "Kreator";
        dynamicButtonStyles = `bg-aksess-skyblue/10 text-aksess-navy border border-aksess-skyblue hover:bg-aksess-skyblue/20 ${baseModeButtonStyles}`;
        break;
      case 'organization':
        dynamicButtonText = "Organisasi";
        dynamicButtonStyles = `bg-aksess-chino/10 text-aksess-navy border border-aksess-chino hover:bg-aksess-chino/20 ${baseModeButtonStyles}`;
        break;
      default: 
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center">
            <button onClick={() => handleLinkClick('landing')} aria-label="Aksess Beranda" className="focus:outline-none">
              <Logo 
              ...
              <button
                ...
                className={`${desktopNavLinkBaseClasses} ${
                  (currentPage === item.target)
                    ? 'font-semibold text-aksess-purple bg-aksess-purple/10 rounded-full'
                    : 'font-medium text-aksess-deep-navy hover:font-semibold hover:text-aksess-purple'
                }`}
          ...
          <div className="hidden md:flex items-center space-x-3">
             {isLoggedIn ? ( 
                ...
                <>
                  ...
                  >
                      <UserCircle size={18} className="text-aksess-purple" /> Masuk / Daftar
                  </button>
                </>
             )}
          </div>
      ...
      {isMobileMenuOpen && (
        <div className={`md:hidden absolute top-0 inset-x-0 min-h-screen ${navbarBgColor}/95 backdrop-blur-lg p-4 pt-20 space-y-2 sm:px-6 z-[-1] flex flex-col animate-fade-in`} id="mobile-menu">
          ...
          {isLoggedIn && ( /* This is the dynamic role button for mobile when logged in */
            <button 
                onClick={handleRoleSpecificBuatEventClick}
                className={`${mobileNavLinkBaseClasses} font-medium text-aksess-deep-navy hover:font-semibold hover:text-aksess-purple flex items-center gap-2`}
                aria-label={dynamicButtonAriaLabel}
            >
                <DynamicButtonIcon size={18} /> {dynamicButtonText}
            </button>
