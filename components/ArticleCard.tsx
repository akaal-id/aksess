...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  const categoryColors: { [key: string]: string } = {
    'Tips & Trik': 'bg-aksess-purple text-white',
    'Studi Kasus': 'bg-aksess-skyblue text-aksess-navy',
    'Wawasan': 'bg-aksess-navy text-white',
  ...
  return (
    <div 
      className="bg-white rounded-xl border border-gray-200 hover:border-aksess-purple/30 overflow-hidden transition-all duration-300 group flex flex-col h-full shadow-sm hover:shadow-lg"
      role="article"
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 
            ...
            className="text-lg font-bold text-aksess-deep-navy mb-2 leading-tight group-hover:text-aksess-gradient-start transition-colors cursor-pointer"
        ...
        <button
          onClick={() => onNavigate(slug)}
          className="mt-auto w-full sm:w-auto self-start bg-transparent text-aksess-purple font-semibold py-2 px-0 rounded-lg
                     transition-all duration-300
                     flex items-center group/button
                     hover:text-aksess-navy"
