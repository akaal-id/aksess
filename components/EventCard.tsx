...
import { EventData, PageName, formatEventTime } from '../AksessApp'; // Renamed import
...
const EventCard: React.FC<EventCardProps> = (props) => {
  ...
  const categoryStyles = {
    B2C: 'bg-aksess-purple text-aksess-deep-navy text-xs',
    B2B: 'bg-aksess-navy text-aksess-white text-xs',
    B2G: 'bg-aksess-skyblue text-aksess-deep-navy text-xs',
  };
  ...
  let finalPriceColorClass = 'text-aksess-skyblue';
  ...
  if (numericPrices && numericPrices.length > 0) {
    ...
    if (!props.displayPrice.toLowerCase().startsWith('rp')) {
      finalPriceColorClass = 'text-aksess-deep-navy'; 
    }
  }


  return (
    <div className="bg-aksess-card-bg rounded-xl border border-aksess-navy/10 hover:border-aksess-purple/20 overflow-hidden transition-all duration-300 group flex flex-col event-card h-full">
      <div className="relative w-full overflow-hidden" style={{ paddingTop: '37.5%' /* 6 / 16 = 0.375 */ }}>
        ...
        <button
          ...
          className="absolute top-4 right-4 p-2.5 bg-white/80 backdrop-blur-sm rounded-full text-aksess-skyblue hover:bg-white/100 transition-colors duration-200 shadow-md"
      ...
      <div className="p-5 flex flex-col flex-grow">
        <h3 className="text-xl font-bold text-aksess-deep-navy mb-2 leading-tight group-hover:text-aksess-gradient-start transition-colors truncate">{name}</h3>
        
        <div className="flex items-center text-sm text-gray-500 mb-1">
          <CalendarDays size={15} className="mr-1.5 text-aksess-purple flex-shrink-0" />
          <span>{dateDisplay}</span>
        </div>
        <div className="flex items-center text-sm text-gray-500 mb-2">
          <Clock size={15} className="mr-1.5 text-aksess-purple flex-shrink-0" />
          <span>{formatEventTime(timeDisplay, timezone)}</span>
        </div>
        <div className="flex items-center text-sm text-gray-500 mb-3"> 
          <MapPin size={15} className="mr-1.5 text-aksess-purple flex-shrink-0" />
          <span>{location}</span>
        </div>
        ...
        <div className="mt-auto"> {/* Pushes content below to the bottom */}
          ...
          <button 
            ...
            className="w-full bg-gradient-to-r from-aksess-purple to-aksess-purple/80 text-white font-semibold py-3 px-4 rounded-lg
                       transition-all duration-300
                       flex items-center justify-center group/button
                       hover:bg-gradient-to-r hover:from-aksess-purple/30 hover:to-aksess-purple/40 hover:text-aksess-purple"
