...
const FloatingHelpButton: React.FC<FloatingHelpButtonProps> = ({ onNavigate }) => {
  ...
  const subButtons = [
    {
      ...
      bgColor: 'bg-aksess-purple',
      textColor: 'text-white',
    },
    {
      label: 'Email Kami',
      icon: Mail,
      action: () => { window.location.href = 'mailto:support@aksess.com?subject=Bantuan%20aksess'; setIsOpen(false); },
      bgColor: 'bg-red-500',
      textColor: 'text-white',
    },
    {
      label: 'WhatsApp Kami',
      icon: MessageCircle,
      action: () => { window.open('https://wa.me/6281234567890?text=Halo%20aksess,%20saya%20butuh%20bantuan.', '_blank'); setIsOpen(false); },
  ...
  return (
    <div ref={fabRef} className="fixed bottom-6 right-6 z-[999]">
      ...
      <button
        onClick={toggleOpen}
        className="w-16 h-16 bg-aksess-navy text-white rounded-full flex items-center justify-center
                   shadow-xl hover:bg-aksess-purple focus:outline-none focus:ring-2
                   focus:ring-offset-2 focus:ring-aksess-skyblue transition-all duration-300 ease-in-out transform hover:scale-110"
