...
// Custom Aksess-themed icons
const aksessBaseIcon = L.Icon.extend({
  options: {
...
const aksesspurpleIcon = new aksessBaseIcon({ iconUrl: 'https://img.icons8.com/ios-filled/50/4b998e/marker.png' }); // purple color
const aksessskyblueIcon = new aksessBaseIcon({ iconUrl: 'https://img.icons8.com/ios-filled/50/ebaf4c/marker.png' }); // skyblue for active
    ...
    if (mapRef.current && markersRef.current) {
      ...
      vendors.forEach(vendor => {
        if (vendor.lat && vendor.lng) {
          const iconToUse = vendor.id === activeVendorId ? aksessskyblueIcon : aksesspurpleIcon;
    ...
    if (mapRef.current) {
      ...
      if (radius && radius > 0) {
        radiusCircleRef.current = L.circle(mapRef.current.getCenter(), { // Use map's current center
          radius: radius,
          color: '#F5AF47', // Aksess skyblue/Accent
          fillColor: '#F5AF47',
          fillOpacity: 0.1,
          weight: 2
        }).addTo(mapRef.current);
