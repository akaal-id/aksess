...
import { EventData, TransactionFormData, formatEventTime } from '../AksessApp';
...
const DetailItem: React.FC<{ icon: React.ElementType; label: string; value?: string | number | null; isLink?: boolean; href?: string }> = ({ icon: Icon, label, value, isLink, href }) => {
    ...
    const content = isLink && href ? (
      <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: '#9747FF', textDecoration: 'underline' }}>{String(value)}</a>
    ) : (
      String(value)
    );
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingBottom: '6mm', marginBottom: '6mm', borderBottom: '0.5px solid #e5e7eb' }}>
        <div>
          ...
          <p style={{ fontSize: '9pt', color: '#4b5563', marginTop: '1.5mm' }}>Tiket Resmi Diterbitkan oleh Aksess</p>
        </div>
      ...
      <div style={{ flexGrow: 1, display: 'grid', gridTemplateColumns: '0.8fr 1.2fr', gap: '6mm', height: '100%' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3mm' }}>
          <div style={{ padding: '3mm', border: '1px solid #9747FF', borderRadius: '3mm', backgroundColor: 'rgba(75,153,142,0.05)' }}>
            <p style={{ fontSize: '8pt', color: '#9747FF', fontWeight: 500, textTransform: 'uppercase', marginBottom: '1mm' }}>Kategori Tiket</p>
            <p style={{ fontSize: '14pt', fontWeight: 'bold', color: '#9747FF', marginBottom: '1mm' }}>{categoryName}</p>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '6mm', borderTop: '0.5px solid #e5e7eb', fontSize: '7.5pt', color: '#6b7280', textAlign: 'center' }}>
        ...
        <p style={{ marginTop: '1mm' }}>&copy; {new Date().getFullYear()} Aksess Event Platform. Info lebih lanjut: www.Aksess.id</p>
