...
import { PageName, EventData, UserRole } from '../../AksessApp'; 
...
const DashboardTopBar: React.FC<DashboardTopBarProps> = ({
  ...
  switch (userRole) {
    case 'visitor':
      dynamicRoleButtonStyles = `bg-aksess-purple/10 text-aksess-purple border border-aksess-purple hover:bg-aksess-purple/20 ${baseDashboardButtonStyles}`;
      break;
    case 'creator':
      dynamicRoleButtonStyles = `bg-aksess-skyblue/10 text-aksess-navy border border-aksess-skyblue hover:bg-aksess-skyblue/20 ${baseDashboardButtonStyles}`;
      break;
    case 'organization':
      dynamicRoleButtonStyles = `bg-aksess-chino/10 text-aksess-navy border border-aksess-chino hover:bg-aksess-chino/20 ${baseDashboardButtonStyles}`;
      break;
    default: 
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <button
              ...
              className="text-gray-500 hover:text-aksess-purple focus:outline-none focus:ring-2 focus:ring-inset focus:ring-aksess-skyblue p-2 rounded-md -ml-2 md:hidden"
              ...
              <ol className="flex items-center space-x-1 text-sm">
                {breadcrumbSegments.map((segment, index) => (
                  <li key={index} className="flex items-center">
                    ...
                    {segment.isCurrent ? (
                      <span className="font-medium text-aksess-navy flex items-center">
                      ...
                      <button
                        ...
                        className="text-gray-500 hover:text-aksess-purple transition-colors flex items-center"
                      >
              ...
              <ol className="flex items-center space-x-1 text-xs truncate">
                {breadcrumbSegments.map((segment, index) => (
                  <li key={`mobile-${index}`} className="flex items-center truncate">
                    ...
                    {segment.isCurrent ? (
                      <span className="font-medium text-aksess-navy truncate flex items-center">
                      ...
                      <button
                        onClick={() => segment.viewId && onSelectView(segment.viewId, segment.data)}
                        className="text-gray-500 hover:text-aksess-purple transition-colors truncate flex items-center"
                      >
