    ...
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <button
          ...
          className="hidden md:block text-gray-500 hover:text-aksess-purple p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-aksess-skyblue"
      ...
      <nav className="flex-1 space-y-1 px-2 py-4 overflow-y-auto">
        {sidebarSections.map((section, sectionIndex) => (
          <div key={section.title} className={sectionIndex > 0 ? 'mt-4 pt-4 border-t border-gray-200' : ''}>
            {isDesktopSidebarOpen && (
              <h3 className="px-3 mb-2 text-xs font-semibold text-aksess-deep-navy/70 tracking-wider uppercase">
                {section.title}
              </h3>
            )}
                  ...
                  className={`w-full flex items-center py-2.5 rounded-md transition-all duration-200 text-sm group
                    ...
                    ${
                      activeViewId === item.id
                        ? 'bg-aksess-purple/20 text-aksess-purple font-medium shadow-sm'
                        : 'text-aksess-deep-navy/70 hover:text-aksess-purple hover:bg-aksess-purple/10'
                    }
                  `}
                  ...
                  <item.icon
                    ...
                    className={`flex-shrink-0 transition-colors
                      ${
                        activeViewId === item.id
                          ? 'text-aksess-purple'
                          : 'text-aksess-deep-navy/60 group-hover:text-aksess-purple'
                      }
                    `}
                  />
