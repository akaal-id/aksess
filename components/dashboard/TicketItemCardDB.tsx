...
import { formatEventTime } from '../../AksessApp';
...
const TicketItemCardDB: React.FC<TicketItemCardDBProps> = ({ ticket, onEdit, onDelete }) => {
  ...
  if (maxQuantity <= 0) { // Should not happen if maxQuantity is always > 0
    ...
    derivedStatusColor = 'text-skyblue-600';
  } else {
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-5">
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-lg font-semibold text-aksess-deep-navy group-hover:text-aksess-purple transition-colors duration-200 flex-grow mr-2 leading-tight">
            {ticket.name}
          </h3>
        ...
        <div className="space-y-2 text-sm text-gray-600">
          ...
          {(!useEventSched && ticket.ticketTimezone) && (
           ...
           <div className="flex justify-between items-center pt-2 mt-2 border-t border-gray-100">
            ...
            <span className="text-lg font-bold text-aksess-purple">{formatCurrency(ticket.price)}</span>
          </div>
        </div>
      </div>
    </div>
