  ...
  return (
    ...
    >
      <div className="bg-white text-aksess-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        ...
        <h2 id="edit-crew-status-modal-title" className="text-xl font-semibold text-aksess-navy mb-2">
        ...
        <p className="text-sm text-gray-500 mb-6">
          ID Crew: <span className="font-medium text-aksess-purple">{crewMember.id}</span>
        </p>

        <div className="space-y-3">
          <p className="text-sm text-gray-700">
            Status Saat Ini: 
            <span className={`ml-1.5 px-2 py-0.5 text-xs font-semibold rounded-full ${crewMember.status === 'Hadir' ? 'bg-green-100 text-green-700' : 'bg-skyblue-100 text-skyblue-700'}`}>
              {crewMember.status}
            </span>
          </p>
