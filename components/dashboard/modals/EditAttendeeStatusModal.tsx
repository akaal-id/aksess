...
const EditAttendeeStatusModal: React.FC<EditAttendeeStatusModalProps> = ({
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="bg-white text-aksess-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        <button
          onClick={onClose}
        ...
        <h2 id="edit-attendee-status-modal-title" className="text-xl font-semibold text-aksess-navy mb-2">
        ...
        <p className="text-sm text-gray-500 mb-6">
          ID Tiket: <span className="font-medium text-aksess-purple">{attendee.ticketId}</span>
        </p>

        <div className="space-y-3">
          <p className="text-sm text-gray-700">
            Status Saat Ini: 
            <span className={`ml-1.5 px-2 py-0.5 text-xs font-semibold rounded-full ${attendee.status === 'Sudah Hadir' ? 'bg-green-100 text-green-700' : 'bg-skyblue-100 text-skyblue-700'}`}>
              {attendee.status}
            </span>
          </p>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 border border-skyblue-500 rounded-lg shadow-sm text-sm font-medium text-skyblue-700 bg-skyblue-50 hover:bg-skyblue-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-skyblue-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 flex justify-end">
          <button
            onClick={onClose}
            type="button"
            className="inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-aksess-purple transition-colors"
          >
            Tutup
          </button>
        </div>
