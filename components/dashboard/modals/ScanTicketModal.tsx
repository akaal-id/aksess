...
const ScanTicketModal: React.FC<ScanTicketModalProps> = ({ 
  ...
  const renderCameraAreaContent = () => {
    switch (cameraAccess) {
      ...
      case 'DENIED':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-skyblue-500 p-4">
    ...
    >
      <div className="bg-white text-aksess-navy p-5 sm:p-6 rounded-xl shadow-2xl w-full max-w-md relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear flex flex-col">
        <div className="flex justify-between items-center pb-3 border-b border-gray-200 mb-4">
          <h2 id="scan-ticket-modal-title" className="text-lg font-semibold text-aksess-navy flex items-center">
            <ScanIcon size={20} className="mr-2 text-aksess-purple" /> Pindai Tiket Pengunjung
          </h2>
        ...
        <div className="flex-grow flex flex-col items-center justify-center">
          ...
          <div className="h-28 text-center flex flex-col justify-center items-center w-full px-2">
            {isSimulatingScan && cameraAccess === 'GRANTED' && (
              <div className="flex flex-col items-center text-aksess-purple">
                <Loader2 size={32} className="animate-spin mb-2" />
        ...
        <div className="mt-6 pt-4 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
           <button
            ...
            className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-5 py-2.5 border border-aksess-purple shadow-sm text-sm font-medium rounded-lg text-aksess-purple bg-white hover:bg-aksess-purple/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-aksess-purple transition-colors disabled:opacity-60"
          >
            <RefreshCw size={16} /> Scan Ulang
          </button>
          <button
            onClick={onClose}
            className="w-full sm:w-auto justify-center inline-flex items-center px-5 py-2.5 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-gray-100 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-aksess-purple transition-colors"
          >
            Tutup
          </button>
        </div>
      ...
      <style>{`
        ...
        .scan-line {
          ...
          width: 100%;
          height: 3px;
          background: linear-gradient(to right, transparent, rgba(75, 153, 142, 0.8), transparent); /* aksess-purple with opacity */
