...
const EditOrderStatusModal: React.FC<EditOrderStatusModalProps> = ({
  ...
  const statusDetails = {
    ...
    Pending: { icon: AlertCircle, color: 'text-skyblue-500', hoverBg: 'hover:bg-skyblue-50' },
  ...
  return (
    ...
    >
      <div className="bg-white text-aksess-navy p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md relative transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        ...
        <h2 id="edit-status-modal-title" className="text-xl font-semibold text-aksess-navy mb-2">
          Ubah Status Pesanan
        </h2>
        <p className="text-sm text-gray-500 mb-6">
          Untuk Pesanan ID: <span className="font-medium text-aksess-purple">{orderId}</span>
        </p>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center text-left p-3 border rounded-lg transition-colors
                                  ${isCurrent 
                                    ? `bg-aksess-purple/10 border-aksess-purple/50 ${color}` 
