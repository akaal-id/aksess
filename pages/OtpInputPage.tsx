...
const OtpInputPage: React.FC<OtpInputPageProps> = ({ 
  ...
  const handleCloseAndGoToLanding = () => {
    // AksessApp will handle closing all auth modals when navigating to landing
    onNavigate('landing');
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-aksess-auth-background p-4">
       <button 
          onClick={handleCloseAndGoToLanding}
          className="absolute top-6 left-6 text-xs text-aksess-auth-text/70 hover:text-aksess-auth-text inline-flex items-center z-20"
          aria-label="Tutup dan kembali ke beranda"
        >
          <X size={18} className="mr-1" /> Tutup
        </button>
      <div className="relative bg-aksess-white w-full max-w-md rounded-2xl shadow-2xl border border-aksess-navy/10 p-8 sm:p-10 text-center transform transition-all duration-300 ease-out scale-95 opacity-0 animate-modal-appear">
        <Logo className="h-10 mb-6 mx-auto" />
        <h1 className="text-2xl font-bold text-aksess-navy mb-2">Masukkan Kode OTP</h1>
        <p className="text-sm text-gray-600 mb-1">
          Kode OTP 6 digit telah dikirimkan ke email Anda:
        </p>
        <p className="text-sm font-semibold text-aksess-purple mb-6 break-all">{email}</p>
            ...
            <input
              ...
              className={`w-10 h-12 sm:w-12 sm:h-14 text-center text-xl sm:text-2xl font-medium border rounded-lg shadow-sm transition-colors bg-white
                          ${error ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' 
                                 : 'border-gray-300 focus:border-aksess-purple/50 focus:ring-aksess-purple/20'} 
        ...
        <div className="text-xs text-gray-500 space-y-1.5">
          ...
          <button
            ...
            className="font-medium text-aksess-auth-primary hover:text-aksess-auth-primary/80 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center"
          >
          ...
          <button 
            ...
            className="font-medium text-aksess-auth-primary hover:text-aksess-auth-primary/80 inline-flex items-center"
          >
