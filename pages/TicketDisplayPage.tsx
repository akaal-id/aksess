...
import { TransactionData, PageName } from '../AksessApp'; // Renamed import
...
const TicketDisplayPage: React.FC<TicketDisplayPageProps> = ({ transactionData, onNavigate }) => {
  ...
  const handleDownloadAllTickets = async () => {
    ...
    pdf.save(`Aksess_E-Tiket_${orderId}.pdf`);
    setIsLoading(false);
  };
  ...
  return (
    <div className="bg-gradient-to-br from-aksess-navy to-aksess-purple min-h-screen py-8 md:py-12 text-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 md:mb-12">
            <button
                ...
                className="flex items-center text-sm text-aksess-skyblue hover:text-opacity-80 font-semibold transition-colors group mb-4 sm:mb-0 self-start sm:self-center"
            >
            ...
            <button
                ...
                className="w-full sm:w-auto flex items-center justify-center gap-2 bg-aksess-skyblue text-aksess-navy font-bold py-2.5 px-6 rounded-lg hover:bg-opacity-90 transition-all duration-300 transform hover:scale-105 shadow-lg disabled:opacity-60 disabled:cursor-not-allowed self-end sm:self-center"
            >
        ...
        {totalTicketsPurchased === 0 && 
            <p className="text-xs text-skyblue-300 text-center -mt-4 mb-6">Tidak ada tiket untuk diunduh.</p>
        }
        ...
        {totalTicketsPurchased > 0 && formData.additionalTicketHolders ? (
         ...
         <p className="text-xs text-center opacity-70 mt-12">
            Pastikan untuk menyimpan tiket ini dengan baik. Aksess tidak bertanggung jawab atas kehilangan atau penyalahgunaan tiket.
          </p>
      </div>
    </div>
  );
