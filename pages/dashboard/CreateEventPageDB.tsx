...
const CreateEventPageDB: React.FC<CreateEventPageDBProps> = ({ onNewEventCreated, onCancel }) => {
  ...
  useEffect(() => {
    setFormData(prev => ({ ...prev, id: `Aksess-EVT-${Date.now()}`})); 
  }, []);
  ...
  const renderSection = (title: string, IconElement: React.ElementType, children: React.ReactNode, sectionId?: string) => (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200" id={sectionId}>
      <div className="flex items-center mb-4 pb-2 border-b border-gray-100">
        <IconElement size={20} className="mr-3 text-aksess-purple" />
        <h2 className="text-xl font-semibold text-aksess-deep-navy">{title}</h2>
      </div>
  ...
  const renderInput = (label: string, name: keyof FormEventData, type: string = "text", placeholder?: string, required: boolean = true, options?: {value: string, label: string}[], disabled: boolean = false, prefix?: string) => {
    ...
    const focusClasses = 'focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50';
    ...
    <form onSubmit={handleSubmit} className="space-y-6 pb-8">
      ...
      {renderSection("Cover Event", ImageIcon, <>
        ...
        {formData.coverImageUrlPreview ? (
          ...
          <div className="mt-2 relative w-full aspect-[16/6] rounded-md overflow-hidden border-2 border-dashed border-aksess-purple/20 bg-white flex flex-col items-center justify-center text-aksess-purple/50">
            <UploadCloud size={48} />
      ...
      {renderSection("Informasi Dasar", Info, <>
        ...
        {renderInput("Slug Link Event", "eventSlug", "text", "cth: konser-band-keren-2025", true, undefined, false, "https://Aksess.id/event/")}
           ...
           <div className="flex items-center gap-2">
            ...
            <label className="flex items-center space-x-1.5 text-sm text-gray-700 mt-1 md:mt-7 whitespace-nowrap">
              <input 
                ...
                className="form-checkbox h-4 w-4 text-aksess-purple rounded focus:ring-aksess-purple/20"
              />
      ...
      <div className="pt-6 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
        ...
        <button type="submit" className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-bold text-white bg-aksess-purple hover:bg-aksess-purple/90 rounded-lg shadow-sm transition-colors">
