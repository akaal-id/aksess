...
const PendapatanDB: React.FC<PendapatanDBProps> = ({ selectedEvent }) => {
  ...
  const handleWithdrawFunds = () => {
    ...
    alert(`Permintaan penarikan dana sebesar ${formatCurrency(pendapatanBersih)} ke rekening ${accountNumber} (${selectedBank}) telah dikirim untuk diproses oleh Admin Aksess.`);
    // Here you would typically make an API call
  };

  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      <h1 className="text-2xl sm:text-3xl font-bold text-aksess-deep-navy mb-2">Pendapatan Event</h1>
      <p className="text-sm text-gray-600 mb-6">Event: <strong className="text-aksess-purple">{selectedEvent.name}</strong></p>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <div className="flex items-center mb-4 pb-2 border-b border-gray-100">
            <CreditCard size={20} className="mr-3 text-aksess-purple" />
            <h2 className="text-xl font-semibold text-aksess-deep-navy">Rekening Bank Anda & Penarikan Dana</h2>
        </div>
        <div className="space-y-4 max-w-xl">
          <div>
            ...
            <input
              ...
              className="w-full py-2 px-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50 transition-colors bg-white"
          ...
          <div>
            ...
            <div className="relative">
                ...
                className="w-full py-2 pl-9 pr-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50 transition-colors appearance-none bg-white"
                >
          ...
          {showBankLainMessage && (
            <div className="p-3 bg-skyblue-50 border-l-4 border-skyblue-400 text-skyblue-700 text-sm">
              <div className="flex">
                <div className="py-1"><AlertCircle className="h-5 w-5 text-skyblue-400 mr-3" /></div>
                <div>
                  Untuk pilihan 'Bank Lain', silakan hubungi Admin Aksess untuk proses penarikan dana secara manual.
                </div>
              </div>
            </div>
          )}
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-aksess-skyblue text-aksess-navy font-bold py-2.5 px-6 rounded-lg hover:bg-opacity-90 transition-colors shadow-md disabled:opacity-60 disabled:cursor-not-allowed"
            >
              <DollarSign size={18} /> Tarik Dana
            </button>
             <p className="text-xs text-gray-500 mt-2">
              Saldo yang dapat ditarik: <strong className="text-aksess-deep-navy">{formatCurrency(pendapatanBersih)}</strong>. 
              Proses penarikan dana memerlukan verifikasi oleh Admin Aksess (1-3 hari kerja).
            </p>
          </div>
        </div>
      </div>
