...
const EventListPageDB: React.FC<EventListPageDBProps> = ({ initialEvents, onNavigate, onSwitchView }) => {
  ...
  const summaryData = useMemo(() => [
    ...
    { title: 'Event Draf', count: augmentedEvents.filter(d=>d.status === 'Draf').length, color: 'skyblue' }, 
  ...
  const renderPageNumbers = () => {
    ...
    return pageNumbersToDisplay.map((number, index) => 
        typeof number === 'string' ? (
            ...
            className={`px-3 py-1 mx-1 rounded-md text-sm transition-colors focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50
                ${currentPage === number 
  ...
  return (
    <div className="p-0 sm:p-2 md:p-4 bg-gray-50 min-h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 px-4 pt-4 sm:px-0 sm:pt-0">
        <h1 className="text-2xl sm:text-3xl font-bold text-aksess-deep-navy mb-4 sm:mb-0">Daftar Event</h1>
        <button
          onClick={() => onSwitchView('createEventView')} 
          className="bg-aksess-purple text-white font-semibold py-2.5 px-5 rounded-lg hover:bg-aksess-purple/90 transition-colors flex items-center gap-2"
        >
        ...
        {summaryData.map((item, index) => (
          <div key={index} className="bg-white p-5 rounded-lg border border-gray-200">
            <div className="flex items-center text-sm text-gray-600 mb-1">
              ...
              {item.color === 'skyblue' && <span className="h-2 w-2 bg-skyblue-400 rounded-full mr-2"></span>}
      ...
      <div className="bg-white p-4 rounded-lg border border-gray-200 mb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <select className="w-full py-2 px-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50 text-sm text-gray-500 bg-white">
