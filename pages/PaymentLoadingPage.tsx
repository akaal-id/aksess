...
import { CheckoutInfo, PageName } from '../AksessApp'; // Renamed import
...
const PaymentLoadingPage: React.FC<PaymentLoadingPageProps> = ({ 
  ...
  const handleCancelPayment = () => {
    // AksessApp's modal will use a generic message, or we can customize it
    // The title and message in AksessApp's ConfirmationModal should be updated for this case.
    // AksessApp's confirmation modal has been updated to handle this specific case message.
    onNavigateRequestWithConfirmation('checkout', checkoutInfoToReturnTo);
  };

  return (
    <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-aksess-white text-aksess-navy p-4 text-center">
      ...
      <button
        ...
        className="mt-12 flex items-center text-sm text-aksess-purple hover:text-aksess-navy font-semibold transition-colors group border border-aksess-purple px-4 py-2 rounded-lg hover:bg-aksess-purple/10"
      >
