...
import { UserRole, PageName, EventData, TicketCategory } from '../AksessApp';
...
const sampleSavedEvents: EventData[] = [
  ...
  },
  {
    ...
    organizerName: 'Organisasi Aksess', summary: 'Konferensi teknologi, inovasi, dan investasi B2B.', googleMapsQuery: 'Hotel Mulia Senayan Jakarta',
...
const manyCreatorDashboardEvents = (creatorName: string): EventData[] => [
  ...
  },
  {
    ...
    location: 'Kopi Studio Aksess, Kemang', posterUrl: 'https://images.unsplash.com/photo-1511920183318-afa7a848f096?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=60',
...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const [creatorData, setCreatorData] = useState<CreatorAccountData>({
    fullName: userName,
    email: userRole === 'creator' ? 'kreator@Aksess.com' : (userRole === 'organization' ? 'org@Aksess.com' : 'visitor@Aksess.com'),
    gender: 'Laki-laki',
  ...
  useEffect(() => {
    if (userName !== creatorData.fullName) {
        setCreatorData(prev => ({ ...prev, fullName: userName, email: userRole === 'creator' ? 'kreator@Aksess.com' : (userRole === 'organization' ? 'org@Aksess.com' : 'visitor@Aksess.com') }));
    }
    ...
    switch (activeView) {
        ...
        return (
          <section aria-labelledby="saved-events-heading" className="mb-10">
            <div className="flex items-center gap-3 mb-6">
              <Bookmark size={28} className="text-aksess-purple" />
              <h2 id="saved-events-heading" className="text-2xl font-semibold text-aksess-navy">Event Tersimpan Saya</h2>
            </div>
      ...
      case 'ticketHistory':
        return (
          <section aria-labelledby="ticket-history-heading">
            <div className="flex items-center gap-3 mb-6">
              <TicketIconLucide size={28} className="text-aksess-purple" />
              <h2 id="ticket-history-heading" className="text-2xl font-semibold text-aksess-navy">Riwayat Pembelian Tiket</h2>
            </div>
            {sampleTicketHistory.length > 0 ? (
              <div className="space-y-4">
                {sampleTicketHistory.map(ticket => (
                  <div key={ticket.id} className="bg-white p-4 sm:p-6 rounded-lg border border-aksess-navy/10 transition-shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h3 className="text-lg font-semibold text-aksess-navy">{ticket.eventName}</h3>
                      <p className="text-sm text-gray-600">Tanggal: {ticket.date} | Harga: {ticket.price}</p>
                      <p className="text-sm text-gray-500 mt-1">Kode Tiket: <span className="font-mono bg-gray-100 px-1 py-0.5 rounded text-aksess-navy">{ticket.ticketCode}</span></p>
                    </div>
                    <div className="flex flex-col items-start sm:items-end mt-2 sm:mt-0">
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${ticket.status === 'Confirmed' ? 'bg-green-100 text-green-700' : 'bg-skyblue-100 text-skyblue-700'}`}>
                        {ticket.status}
                      </span>
                      <button className="mt-2 text-sm text-aksess-purple hover:underline">Lihat Detail Tiket</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      case 'accountInfo':
        return <AccountInfoDB
                ...
                />;
      default:
        return <div className="p-6"><h2 className="text-2xl font-semibold text-aksess-navy mb-4">Dashboard Visitor</h2><p className="text-gray-700">Selamat datang! Pilih menu untuk melihat aktivitas Anda.</p></div>;
    }
  };
    ...
    switch (activeView) {
      ...
      case 'editEventView':
        ...
        return <div className="p-6 text-center">
                ...
                <button onClick={() => handleSwitchView('daftarEvent')} className="bg-aksess-purple text-white px-5 py-2.5 rounded-lg hover:bg-opacity-90 transition-colors flex items-center justify-center gap-2 mx-auto">
