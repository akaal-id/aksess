...
const PasswordStrengthMeter: React.FC<{ password?: string }> = ({ password = "" }) => {
  const calculateStrength = (pass: string): PasswordStrength => {
    ...
    if (score === 2) return { score: 2, text: "Sedang", colorClass: "bg-skyblue-500" };
  ...
  return (
    <div className="mt-1.5">
      ...
      <p className={`text-xs mt-1 ${strength.score === 1 ? 'text-red-500' : strength.score === 2 ? 'text-skyblue-600' : 'text-green-600'}`}>
        {strength.text}
      </p>
    </div>
  );
...
const CreatorAuthPage: React.FC<CreatorAuthPageProps> = ({ 
  ...
  const inputBaseClasses = "w-full py-2.5 pl-10 pr-3 bg-white border rounded-lg shadow-sm focus:ring-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50 transition-colors placeholder-gray-400 text-sm";


  return (
    <div className="min-h-screen flex bg-aksess-auth-background">
      <div className="hidden lg:flex lg:w-2/5 bg-aksess-white flex-col items-center justify-center p-12 text-aksess-navy relative overflow-hidden">
        <div className="w-full max-w-md z-10">
          <Logo className="h-12 mb-8" />
          <h1 className="text-3xl font-bold mb-3">Welcome to Aksess Event Creator Dashboard</h1>
          ...
          <div className="space-y-4">
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start">
                <benefit.icon className="h-6 w-6 text-aksess-purple mr-3 mt-0.5 flex-shrink-0" />
      ...
      <div className="w-full lg:w-3/5 flex items-center justify-center p-6 sm:p-8 md:p-12 relative">
        <button 
          ...
          className="absolute top-6 right-6 text-xs text-aksess-auth-text/70 hover:text-aksess-auth-text inline-flex items-center"
        >
          <ArrowLeft size={14} className="mr-1" /> Kembali ke Halaman Utama
        </button>

        <div className="w-full max-w-md bg-aksess-white p-8 sm:p-10 rounded-xl shadow-2xl">
          <div className="text-center lg:hidden mb-6">
             <Logo className="h-10 mb-4 mx-auto" />
          </div>
          
          <h1 className="text-2xl sm:text-3xl font-bold text-aksess-navy text-center mb-6">
            {panelTitle}
          </h1>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-5">
            ...
            {mode === 'login' && (
              <div className="text-right">
                <a href="#" className="text-xs font-medium text-aksess-auth-primary hover:text-aksess-auth-primary/80">Lupa password?</a>
              </div>
            )}
          ...
          <div className="mt-6">
            ...
            <div className="mt-4">
              <button type="button" className="w-full inline-flex justify-center items-center py-2.5 px-4 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-aksess-purple/20 focus:border-aksess-purple/50">
          ...
          <p className="mt-8 text-center text-xs text-gray-500">
            ...
            <button onClick={() => setMode(mode === 'login' ? 'signup' : 'login')} className="font-medium text-aksess-auth-primary hover:text-aksess-auth-primary/80 ml-1">
